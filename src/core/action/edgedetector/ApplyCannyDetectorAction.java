...
public class ApplyCannyDetectorAction {
    ...
    public CustomImage execute(CustomImage filteredImage, int t1, int t2) {
        ...
        int[][] derivativesAbsoluteSumMatrix = this.imageOperationsService.calculateAbsoluteSum(sobelXDerivative, sobelYDerivative).getRedChannel(); //Again, easily extended for 3 channels
    ...
    private int markEdgeIfItsConnectedToAnotherEdge(int[][] roughSingleEdgedMatrix, int x, int y) {
        ...
        for(RGB neighbor : neighbors) {
            if (neighbor.getRed() == 255) return 255;
        }
        return 0;
    ...
    private int chooseImageAngle(double realAngle) {
        ...
        if (realAngle < 22.5 || realAngle >= 157.5) return 0;
        else if (realAngle >= 22.5 && realAngle < 67.5) return 45;
        else if (realAngle >= 67.5 && realAngle < 112.5) return 90;
        else if (realAngle >= 112.5 && realAngle < 157.5) return 135;
        else throw new RuntimeException ("Unconsistent angle"); //This should never happen, since realAngle is contained within the first or second quadrant.
